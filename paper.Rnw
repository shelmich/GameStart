...
\hh{XXX what do you mean by first step? If you need iterations, use $r_{u,i}^{(n)}$ or something similar. If there is an iteration, make sure to give a convergence criterion. 

Ah, ok - I see - I think what is going on is that you first get rid of the individuals' effects, and then of the games' effect. Write that out in two separate pieces. Give them appropriate names :)  - then you will have a much easier time describing what you are doing than in the paragraph below, where you are switching out us and is.
}
\hh{XXX: doesn't this error need to be indexed as well? could we call the error $\epsilon$ and make away with the text in the formula? }
\hh{XXX: generally:  no future tense in the write up. The CC is in the here and now :)
}

Where for the first step, $r_{u,i}$ \sout{will be} \hh{is} the rating from user $u$ for game $i$ and $x_{u,i}$ will be an indicator variable that is 1 if player $u$ has rated game $i$ and 0 otherwise. In the second step $r_{u,i}$ will be the residual from the first portion, the subscript on $\theta$ becomes $i$ and $x_{u,i}$ will be an indicator variable that is 1 if the rating is for game $i$ and 0 otherwise.

\hh{XXX it is $\theta_u$, not $\theta$. }
\hh{XXX Why do you call $\alpha$ a fitting parameter and not a shrinking parameter?}

To get an estimate of $\theta$ at either step, we'll use the average of the nonzero values which are shrunk by the fitting parameter $\alpha$ to avoid overfitting (motivation for using $\alpha$ in this manner can be found in \cite{scalecollab}). The sum is over either all ratings for user $u$ or all ratings for game $i$, depending on whether you're estimating $\theta_u$ or $\theta_i$. Either way, the structure is the same. \hh{Again, write this out in two separate formulae.}
\begin{equation}
\widehat{\theta_u} = \frac{\sum r_{u,i}}{n_u + \alpha}
\end{equation}
...
There are three parameters that the model relies on: The two $\alpha$ values that balance out biasedness in the global effects section, and the $k$ in the k-nearest neighbors portion of the algorithm. The $\alpha$'s will be done globally (that is, every individual will use the same values for $\alpha_1$ and $\alpha_2$), while the values for $k$ will be done locally (each user will have their own value of $k$). 

\subsubsection{Optimizing $\alpha$}
To optimize the $\alpha$ values, we'll take all of the ratings for all players and subtract off the mean of all of the ratings (this is the first step in the globalized residual process). This leaves us with three variables: The userid, the gameid and the residual. We'll randomly split all of the data into 10 equally sized buckets. We'll use each of these buckets as a test group, where we put the other buckets together, fit a model using a canidate value for $\alpha$, and then fit the values in the test group. From each test group, we'll then find the mean square error, and then average those together from all 10 groups. Taking the square root of that average, we can get the RMSE for each canidate value for $\alpha$. Finally, we'll proceed using the canidate value for $\alpha$ with the lowest RMSE. A graph of this process can be seen \hh{in figure~\ref{fig:alpha}}, with the domain truncated for easier visibility.

\begin{figure}
<<game_alpha_optim, echo = F>>=
load("game_graph")
game_graph
@
\caption{\label{fig:alpha} You need a caption here - what is shown?}
\end{figure}

<<player_alpha_optim, echo = F>>=
load("player_graph")
player_graph
@
