...
\begin{equation}
\text{RMSE} = \sqrt{\frac{\sum (y - \widehat{y})^2}{n}}
\end{equation}
...
To optimize the tuning parameter $\alpha$ values, we follow the model fitting procedure up to the point in which the paramter is used, estimate the model parameter using a grid of possible values for the turning parameter, and then find which value for the tuning parameter yields the model parameter estimate which has the lowest RMSE (equation (1)).
...
plyr_n_plot
@
\caption{\label{fig:ratn} Frequency of log number of ratings for games (left) amd players (right).}
\end{figure}
...
\caption{\label{fig:blanksc} A blank login screen}
\end{figure}
\end{center}
...
Another possible step is to generalize the whole recommendation system procedure, so that if you had rating and classifing information on any manner of subjects, you could implement a recommendation system with little to no effort. This could have a whole host of applications from what restaurants you might enjoy to what books you might like to read. 

Finally, there are hosts of other recommender systems out there. Augmenting this approach with additional models as done in the Netflix Prize papers would undoubably be a logical step forward for this kind of modeling. 
