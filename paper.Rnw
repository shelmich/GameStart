...
From what we've produced above in equations (2)-(9), of the estimated rating for user $u$ and game $i$ is given by:
...
In an implementation of this, the application  begins by having the user enter the username they've registered on \url{boardgamegeek.com}. As seen in (~\ref{fig:blanksc}), there is an area to enter the users information in, and a button just below that, when clicked, begins the process.
\begin{center}
\begin{figure}\centering
