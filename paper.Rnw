...
From the RMSE of their probe set, the difference between using only user/movie effcts and using all 14 effects was only .02, a decrease of about 2\%. Noting this negligible increase in accuracy and recognizing my lack of significant computing resources, we will only use user and game effects.
...
\hh{XXX: generally:  no future tense in the write up. The CC is in the here and now :)
}


\hh{XXX it is $\theta_u$, not $\theta$. }
\hh{XXX Why do you call $\alpha$ a fitting parameter and not a shrinking parameter?}
...
To optimize the $\alpha$ values, we'll follow the model fitting procedure up to the point in which the paramter is used, estimate the model parameter using a grid of possible values for the turning parameter, and then find which value for the tuning parameter yields the model parameter estimate which has the lowest root mean square error (RMSE). RMSE is calculated by finding the difference between the rating and the rating that is estimated using only the global effects parameter(s).

The first $\alpha$ to be tuned is $\alpha_{game}$. The data is randomly divided into 10 separate sections of roughly equal size. Then for each of the 10 sections, we'll treat the section as a test set and the other 9 to use as a training set. $\widehat{\theta}_g$ is fit first, and then from a grid of $\alpha_{game}$ values (where $\alpha_{game} \in [1,10000]$), a value of $\widehat{\theta}_i$ is produced for each of the $\alpha_{game}$ in the grid. For each of the $\widehat{\theta}_i$ that are estimated, we'll use the section that wasn't in the training set, the RMSE is calculated by:
\begin{equation}
\text{RMSE} = \frac{ \sum r_{u,i}^{(1)} - (\theta_g + \theta_i)}{n}
\end{equation}

where we sum over all rated games in the test set, and $n$ is the number of ratings in the test set. The value of $\alpha_{game}$ that has the lowest RMSE will be used. in figure ~\ref{fig:gamealpha}, we see the optimal value for $\alpha_{game}$ sits in a nice trough, with the minimum at $\alpha_{game} = 1.802303$.

\begin{center}
\begin{figure}
<<game_alpha_optim, echo = F, fig.width = 4, fig.height = 4 >>=
load("game_graph")
game_graph
@
...
\begin{figure}
<<player_alpha_optim, echo = F, fig.width = 4, fig.height = 4>>=
load("player_graph")
player_graph
@
...
\begin{figure}
<<hists, echo = F, fig.width = 4, fig.height = 4>>=
load("game_n_rats")
load("plyr_n_rats")
game_n_plot
plyr_n_plot
@
\caption{\label{fig:ratn} Frequency of number of ratings for players and games.}
\end{figure}
